...
class ProbabilityVector:
    def __init__(self, probabilities: dict):
        ...
        assert abs(sum(probs) - 1.0) < 1e-12 ## probability must sum up to 1.
        assert len(list(filter(lambda x: 0 <= x <= 1, probs))) == len(probs) ## probability must be numbers from 0~1 interval
    ...
    def ___rmul__(self, other) -> np.ndarray:
...
class ProbabilityMatrix:
    ...
    def __getitem(self, observables: str) -> np.ndarray:
...
# a2 = ProbabilityVector({'rain': 0.1, 'sun': 0.9})
# print(a1)
# print(a2)
...
# b1 = ProbabilityVector({'OS':0.1, '1M':0.4, '2L':0.5})
# b2 = ProbabilityVector({'OS':0.7, '1M':0.2, '2L':0.1})
# B = ProbabilityMatrix({'OH':b1, '1C':b2})
# print(B)
# print(B.df)
